### String References in Preprocessor Instructions

Preprocessor instructions that reference strings (like PPDEF, PPUNDEF, PPCOND) use string table indices rather than embedding literals:

```
; Define DEBUG symbol (actual binary encoding)
[0xF1][0x02][0xF4][0x00][string_table_index_for_DEBUG]

; Check if DEBUG is defined
[0xF0][0x02][CONDITION_TYPE:DEFINED][0xF4][0x00][string_table_index_for_DEBUG]
```

This applies to:
- Symbol names in PPDEF and PPUNDEF
- File names in PPINC
- String literals in macro bodies
- Any other string value used by preprocessor instructions

When viewing disassembled COIL code or examples, the strings may be shown explicitly for readability, but the actual binary representation uses string table indices.# Preprocessor Extension
