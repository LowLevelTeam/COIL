...
typedef enum coil_section_type_e {
  ...
  COIL_SECTION_NATIVE = 7,      ///< Alias of Target for backwards compatibility
  COIL_SECTION_CBC = 8,         ///< Section with specific target architecture COIL Byte Code
} coil_section_type_t;
...
typedef enum coil_section_flag_e {
  ...
  COIL_SECTION_CBC         = 1 << 6,   ///< Section with specific target architecture COIL Byte Code
} coil_section_flag_t;
...
// COIL Byte Code is the next stage after COIL,
// CBC is utilized for fast JIT of similar devices and architectures under a unified instruction set.
// CBC is varied by each vastly different set of devices. CPUs like arm and x86 could share a CBC instruction set but
// the riscv instruction set lacking flags would have to be classed as something seperate.
